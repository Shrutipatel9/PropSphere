...
export class MortgageCoreCalcComponent implements AfterViewInit {
  ...
  constructor(
    ...
    private customValidator: CustomValidatorsDirective
  ) {
      ...
      const balance = isLast
        ...
        : Number(
            (
              ...
              Number(report.principal.toFixed(2))
            ).toFixed(2)
          );
      const int = Number(
        (
          Number(report.balance.toFixed(2)) *
          (interest / 100 / this.payPerYear)
        ).toFixed(2)
      );
      const principal = Number(
        (Number(report.payment.toFixed(2)) - int).toFixed(2)
      );
  ...
  private monthlyPayCalculate(
    ...
    return {
      ...
      lifetimeTotal: (total * payPerTotal)
        .toString()
        .replace(/\B(?=(\d{3})+(?!\d))/g, ','),
    };
  }
